...
export default class PlayerController {
    ...
    public static async createPlayer(req: Request, res: Response): Promise<void> {
        try {
            ...
            res.status(500).json({ message: "There was an error creating the player" });
        }
    };

    // Controlador per modificar el nom d'un jugador PUT
    public static async updatePlayerName(req: Request, res: Response): Promise<void> {
        try {
            ...
            res.status(500).json({ message: "There was an error updating the player's name" });
        }
    };

    // Controlador per obtenir tots els jugadors GET
    public static async getAllPlayers(req: Request, res: Response): Promise<void> {
        try {
            const players = await Player.findAll();
            res.status(200).json(players);
        } catch (error) {
            console.error("Error getting players:", error);
            res.status(500).json({ message: "There was an error getting the players" });
        }
    }
}
